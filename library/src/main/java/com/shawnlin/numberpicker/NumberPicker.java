...
public class NumberPicker extends LinearLayout {
     ...
     */
    private static final int SNAP_SCROLL_DURATION = 150;
...
//        }
    ...
    public int smoothScrollToWithRandomNumber(int number) {
        final int currentPosition = getSelectorIndices()[mWheelMiddleItemIndex];
        ...
        for (int i = 0; i < number; i++) {
            handler1.postDelayed(new Runnable() {
                @Override
                public void run() {
                    smoothScroll(false, 1);
                }
            }, SNAP_SCROLL_DURATION * i);
        }
        return SNAP_SCROLL_DURATION * (number - 1);
    }
